<!-- source: https://docs.x402.org/advanced-concepts/lifecycle-hooks.md -->
<!-- fetched: 2026-09-15 -->
...
Every settle hook receives a `phase` field on its context indicating which settle invocation is running:

| Phase            | When it runs                                                                        |
| ---------------- | ----------------------------------------------------------------------------------- |
| `before-handler` | Settlement before the resource handler (e.g. `upfront`, `escrow` deposit)           |
| `after-handler`  | Settlement after the resource handler (e.g. `authorization` charge, `escrow` claim) |
| `cancel`         | Refund or close settle from a verified-payment cancellation                         |

The `escrow` payment flow settles twice — once before the handler and once after — so `onBeforeSettle`, `onAfterSettle`, `onSettleFailure`, and `enrichSettlementPayload` / `enrichSettlementResponse` each fire once per settle. If your hook has side effects that must not run twice (for example, sending a receipt email), branch on `context.phase`:

<Tabs>
  <Tab title="TypeScript">
    ```typescript theme={null}
    server.onAfterSettle(async (context) => {
      // Only send receipt on the final charge, not the escrow deposit
  ...
  <Tab title="Python">
    ```python theme={null}
    async def send_receipt_on_final_charge(context):
        # Only send receipt on the final charge, not the escrow deposit
    ...
    server.on_after_settle(send_receipt_on_final_charge)
    ```
  </Tab>
...
For static USD caps and asset allowlists, use [`spendControls`](/getting-started/quickstart-for-buyers#spend-controls) on the client config — it runs before hooks and is the preferred way to enforce spend limits. Use `onBeforePaymentCreation` for dynamic or interactive checks (e.g. prompting the user for approval).
...
<Tabs>
  <Tab title="TypeScript">
    ...
    client.onBeforePaymentCreation(async (context) => {
      // Use this hook for interactive or dynamic checks.
      // For static USD caps and asset allowlists, use spendControls instead.
      const approved = await promptUser(context.selectedRequirements);
      if (!approved) {
        return { abort: true, reason: "User declined payment" };
      }
    });
    ```
  </Tab>
