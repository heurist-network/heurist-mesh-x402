<!-- source: https://docs.x402.org/extensions/bazaar.md -->
<!-- fetched: 2026-09-15 -->
...
Whether and how a resource appears in a facilitator's catalog is an implementation detail of the **facilitator
operator**, an external 3rd party service provider.
...
validate bazaar data, instead of hand-rolling `PaymentRequired`, `PaymentPayload` or extension schemas.
...
2. **The echoed declaration is not spec-conformant.** Servers advertise bazaar
   ...
   * `$ref` or `$id` values in the `schema` field that are not same-document
     JSON Pointer fragments (i.e. do not start with `#`) — external references
     such as `https://...`, `file://...`, or relative URIs are rejected to
     prevent SSRF/LFI (CWE-918). Only in-document references like
     `"$ref": "#/definitions/foo"` are allowed.
