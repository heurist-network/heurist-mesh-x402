<!-- source: https://docs.x402.org/extensions/overview.md -->
<!-- fetched: 2026-09-15 -->
...
| Builder Code                   |            —            |  ✅ (declares app code + schema) |              —             | ✅ (encodes ERC-8021 calldata suffix) |
...
| [Builder Code](./builder-code)                                    | Server + Client + Facilitator | On-chain attribution tracking via ERC-8021 Schema 2. Appends CBOR-encoded builder codes to settlement transaction calldata.  | TypeScript, Go, Python |
...
| [Sign-In-With-X](./sign-in-with-x)                                | Server + Client               | CAIP-122 wallet authentication. Lets clients prove wallet ownership to access previously purchased content without repaying. | TypeScript, Go, Python |
...
const myExtension: ResourceServerExtension = {
  ...
  // Mark fields that are regenerated on every 402 response so they are
  // excluded from client echo validation (extension_echo_mismatch check).
  // Omit this property when all info fields are static committed terms.
  dynamicInfoFields: ["timestamp"],

  enrichPaymentRequiredResponse: async (declaration, context) => {
    // Add custom data to the 402 response
    return { info: { customField: "value", timestamp: Date.now() } };
  },
