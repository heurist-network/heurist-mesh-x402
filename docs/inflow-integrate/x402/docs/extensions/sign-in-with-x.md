<!-- source: https://docs.x402.org/extensions/sign-in-with-x.md -->
<!-- fetched: 2026-09-15 -->
...
<Tabs>
  ...
  <Tab title="Go">
    ```go theme={null}
    signinwithx.MustCreateResourceServerExtension(signinwithx.ServerOptions{
        Storage: storage,
        Origin:  "https://api.example.com",
    })
    ```
  ...
  <Tab title="TypeScript">
    ...
    async function handleRequest(request: Request) {
      ...
      if (!validation.isValid) {
        return { error: validation.invalidMessage, code: validation.invalidReason };
      }

      // Verify signature and recover address
      const verification = await verifySIWxSignature(payload);
      if (!verification.isValid) {
        return { error: verification.invalidMessage, code: verification.invalidReason };
      }

      // verification.payer is the verified wallet address
      // Grant access for auth-only routes or if wallet has paid
      const isAuthOnly = await checkIfAuthOnlyRoute(request);
      const hasPaid = await checkPaymentHistory(verification.payer);
...
<Tabs>
  <Tab title="TypeScript">
    ...
    // 4. Create signed payload (pass the 402 response URL for origin binding)
...
Go: `DeclareExtension(DeclareOptions{Statement, Version, Networks, ExpirationSeconds})`

Python: `declare_siwx_extension(DeclareSIWxOptions(...))`
...
Creates the server extension that enriches SIWX challenges, records successful payments, and verifies HTTP SIWX proofs for declared routes. Requires `origin` / `Origin`. Internally uses settle and request hooks.

Go: `CreateResourceServerExtension(ServerOptions{Storage, Origin, VerifyOptions, OnEvent})`

Python: `create_siwx_resource_server_extension(CreateSIWxHookOptions(storage=..., origin=...))`
...
// Returns: { isValid: true } | { isValid: false; invalidReason: SIWxValidationCode; invalidMessage: string }
```

Go: `ValidateMessage(payload, *url.URL, ValidationOptions)`

Python: `validate_siwx_message(message, expected_origin: str, options=None)`
...
// Returns: { isValid: true; payer: string } | { isValid: false; invalidReason: SIWxVerifyCode; invalidMessage: string }
```
...
**Problem:** `verifySIWxSignature` returns `{ isValid: false }`.

Check `invalidReason` for the specific failure code (e.g., `invalid_siwx_signature`, `invalid_siwx_unsupported_chain`).
...
**Problem:** `validateSIWxMessage` returns `{ isValid: false }`.

Check `invalidReason` for the specific failure code (e.g., `invalid_siwx_expired`, `invalid_siwx_domain_mismatch`).
