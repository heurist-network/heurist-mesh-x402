<!-- source: https://docs.x402.org/schemes/batch-settlement.md -->
<!-- fetched: 2026-09-15 -->
...
| Field                                     | Description                                                                                                                                                             |
| ----------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `depositMultiplier` / `DepositMultiplier` | Deposits `amount x multiplier` for the advertised per-request maximum. The default is `5`; the minimum is `3`. Used as a fallback when no `minDeposit` hint is present. |
| `depositStrategy` / `DepositStrategy`     | Optional callback for caps, dynamic deposits, or opting out of automatic top-up.                                                                                        |

#### Server-announced `minDeposit` hint
    ...
    app.use(paymentMiddleware({
      "GET /weather": {
        accepts: {
          ...
          payTo: receiverAddress,
          extra: {
            minDeposit: "$1.00",  // money string for default assets
            // or: minDeposit: "1000000",  // atomic integer string
          },
        },
      },
    }, resourceServer));
...
Use `depositStrategy` for fine-grained control: cap deposits, skip top-ups, or return a custom amount. The callback receives the computed deposit amount (already incorporating the `minDeposit` hint) and can return a capped value, `false` to skip the deposit, or `undefined` to accept the computed amount.
...
Every channel commits to a `receiverAuthorizer`, which signs claim and refund authorizations. The server must have a `receiverAuthorizer` available from one of two sources:
...
| Facilitator-delegated authorizer        | Simpler to run. Omit `receiverAuthorizerSigner` and the server picks up the address advertised by the facilitator's `/supported` response. If you switch facilitators, claim and refund old channels before opening new ones. |

The server validates this configuration at startup. If no `receiverAuthorizerSigner` is configured and the facilitator does not advertise a valid `receiverAuthorizer` in its `/supported` response, the server will throw an error during `initialize()` rather than failing on the first protected request.
...
The batch-settlement facilitator scheme accepts an optional `authorizerSigner`. Pass one to enable facilitator-delegated authorization; omit it to require servers to supply their own.
...
<Tabs>
  <Tab title="Go">
    ...
    evmSigner, _ := evmsigners.NewFacilitatorSignerFromPrivateKey(os.Getenv("EVM_PRIVATE_KEY"), rpcURL)

    // Optional: provide an authorizer key to enable facilitator-delegated authorization.
    // When nil, no receiverAuthorizer is advertised and servers must supply their own.
    var authorizer batchsettlement.AuthorizerSigner
    if authKey := os.Getenv("EVM_RECEIVER_AUTHORIZER_PRIVATE_KEY"); authKey != "" {
        authorizer, _ = evmsigners.NewAuthorizerSignerFromPrivateKey(authKey)
    }
  ...
  <Tab title="Python">
    ...
    # Optional: provide an authorizer key to enable facilitator-delegated authorization.
    # When None, no receiverAuthorizer is advertised and servers must supply their own.
...
* [Go facilitator](https://github.com/x402-foundation/x402/tree/main/examples/go/facilitator/batch-settlement)
...
* [`batch-settlement` SVM spec](https://github.com/x402-foundation/x402/blob/main/specs/schemes/batch-settlement/scheme_batch_settlement_svm.md)
