<!-- source: https://docs.x402.org/schemes/overview.md -->
<!-- fetched: 2026-09-15 -->
...
Every scheme declares which payment flows it supports per asset transfer method. A payment flow controls when verification and settlement happen relative to the resource handler:

| Flow            | Verify before handler | Settle before handler | Settle after handler | Description                                                                                                                                                                              |
| --------------- | --------------------- | --------------------- | -------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `authorization` | ✅                     | ❌                     | ✅                    | Verify the authorization signature before serving the response; charge after. Default for all current schemes.                                                                           |
| `upfront`       | ❌                     | ✅                     | ❌                    | Settle before the handler runs; no after-handler settle. Payment is committed before the resource is served.                                                                             |
| `escrow`        | ❌                     | ✅                     | ✅                    | Settle before the handler (deposit) and again after (claim). Settle hooks fire once per settle — see [Lifecycle Hooks](/advanced-concepts/lifecycle-hooks) for how to branch on `phase`. |

The server resolves the flow from the scheme's `paymentFlows` table using the `assetTransferMethod` in the payment requirements. When the resolved flow is not `authorization`, the server signals it to clients via `extra.paymentFlow` in the 402 response.

The `exact` scheme supports both `authorization` (default) and `upfront` flows on all networks. Servers opt in per route by setting `extra.paymentFlow: "upfront"` in the route's `accepts` config. The `upto` and `batch-settlement` schemes support `authorization` only.
